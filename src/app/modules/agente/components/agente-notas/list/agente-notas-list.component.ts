...
export class AgenteNotasListComponent extends ABMListComponent {
    ...
    @ViewChild(AgenteNotasCreateUpdateComponent)
    createUpdateComponent: AgenteNotasCreateUpdateComponent;
    ...
    public onItemDelete(item: any) {
        this.notaService.delete(item._id).subscribe(
            (result) => {
    ...
    createCreateUpdateNotasComponent(action: String, editable: Boolean, item?) {
        ...
        if (action == "create") {
            ...
            this.componentRef.instance.success.subscribe((nota) => {
