...
export class AgenteReactivarComponent {
    @Input() agente: Agente;
    @Input()
    item: HistoriaAgenteReactivacion = new HistoriaAgenteReactivacion();
    ...
    @ViewChild(HistoriaLaboralFormComponent)
    historiaFormComponent: HistoriaLaboralFormComponent;

    constructor(private agenteService: AgenteService, public plex: Plex) {}
    ...
    public guardar() {
        if (this.historiaFormComponent.invalid()) {
            ...
            console.log(datosReactivacion);
            this.agenteService
                .reactivar(this.agente, datosReactivacion)
                .subscribe((agente) => {
                    // Try to save files
                    this.historiaFormComponent.datosNormaLegal.fileManager.saveFileChanges(
