...
export class ParteAgenteListComponent extends ABMListComponent {
    ...
    public ngOnInit() {
        this.parteJustificacionService.get({}).subscribe((data) => {
    ...
    search(searchParams) {
        ...
        this.objectService
            .get(this.dataService, { ...searchParams, ...this.sortParams })
            .subscribe(
                (objects) => {
                    return objects && objects.length
                        ? this.searchPartesAgentes(objects[0])
    ...
    createPartes(params) {
        if (!params["fecha"] || !params["ubicacion.codigo"])
            return this.searchPartesAgentes(null);
        this.ubicacionService
            .getByCodigo(params["ubicacion.codigo"])
            .subscribe((ubi) => {
                if (ubi) {
                    const ubicacion = {
                        codigo: ubi.codigo,
    ...
    public refreshParte(parte: Parte) {
        ...
        if (
            parte &&
            parte.estado &&
            parte.estado.nombre == "Presentación total"
        ) {
    ...
    public onSavePartes() {
        this.parteService
            .guardar(this.parteToUpdate, this.items)
            .subscribe((data) => {
                // TODO Mejorar el manejo de errores
                this.plex
                    .info(
                        "info",
                        `Parte actualizado correctamente. El parte
    ...
    public onConfirmarPartes() {
        this.parteService
            .confirmar(this.parteToUpdate, this.items)
            .subscribe((data) => {
                // TODO Mejorar el manejo de errores
                this.plex
                    .info(
                        "info",
                        `Parte actualizado y confirmado correctamente.
        ...
        this.parteService
            ...
            .subscribe((data) => {
                ...
                this.plex
                    .info(
                        "info",
                        `Parte editado correctamente. Se ha notificado
